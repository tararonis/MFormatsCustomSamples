...
namespace CGEditor_WinForms.CustomPropertyEditors
{
    public partial class GradientEditor : UserControl
    {
        ...
        private void GetColorAngle()
        {
            SpinnerGradientAngle.Value = (decimal)m_Item.ColorAngle;
        }
        ...
        private void SetColor()
        {
            if (m_Item != null && m_bChangedByUI)
            {
                ...
                m_Item.Color = strCGGradient;
                m_bAceptExternalUpdate = true;
            }
        }

        private void SetColorAngle()
        {
            if (m_Item != null && m_bChangedByUI)
            {
                UndoRedoManager.UndoRedoManager.Instance().Push<HistoryItem>(m_Editor.UndoAction, new HistoryItem(m_Editor.CurrentState, ((CGBaseItem)m_Item).ID));
                double dblAngle = (double)SpinnerGradientAngle.Value;
                m_bAceptExternalUpdate = false;
                m_Item.ColorAngle = dblAngle;
                m_bAceptExternalUpdate = true;
            }
        }
        ...
        void External_ItemChanged(object sender, ItemChangedArgs e)
        {
            if (m_bAceptExternalUpdate)
            {
                if (e.PropertyName == "color")
                    GetColor();
                else if (e.PropertyName == "color-angle")
                    GetColorAngle();
        ...
        private void SpinnerGradientAngle_ValueChanged(object sender, EventArgs e)
        {
            SetColorAngle();
        }
