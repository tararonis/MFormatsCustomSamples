...
namespace MF_CreateBitmap_32888
{
    public partial class Form1 : Form
    {
        ...
        int index = 0;
        public void GetNextFrame(out MFFrame pFrame)
        {
            index++;
            pFrame = null;
            ...
            if (true)
            {
                Bitmap b = new Bitmap(@"M:\TEST_VIDEOS\!!GoodBackGround.jpg");
                _factory.MFFrameCreateFromHBITMAP((long)b.GetHbitmap(Color.Black), out pFrame, "");

                b.Dispose();
                b = null;

                var pTime = new M_TIME();
                if (index == index) pTime.eFFlags = eMFrameFlags.eMFF_Last;
                pTime.rtStartTime = currentFrameTime;
                pTime.rtEndTime = currentFrameTime + frameDuration;

                currentFrameTime += frameDuration;
                currentFrameTime += 1;

                pFrame.MFTimeSet(ref pTime);
            }
